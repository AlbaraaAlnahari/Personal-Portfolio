...
const CV_AR = {
  ...
  skills: [
    ...
    { label: "الويب", items: "HTML, CSS, TypeScript, JavaScript" },
    {
      ...
      items: "Pandas, NumPy, Data Analysis, OpenCV, AI APIs",
    },
...
export function ResumePreviewViewer() {
  ...
  const c = warm
    ? {
        shell: "#efe9dc",
        bar: "rgba(120,95,55,0.14)",
        canvas: "#e7e0cf",
        sheet: "#faf6ea",
        ink: "#1d2740",
        sub: "#414c66",
        faint: "#727a90",
        line: "rgba(29,39,64,0.12)",
        thumb: "rgba(29,39,64,0.28)",
        sheetShadow: "0 16px 40px rgba(40,30,10,0.14)",
      }
    : {
        shell: "#0c111e",
        bar: "rgba(140,165,215,0.14)",
        canvas: "#080c16",
        sheet: "#111827",
        ink: "#eef3ff",
        sub: "#b6c2d8",
        faint: "#8492ab",
        line: "rgba(130,165,215,0.15)",
        thumb: "rgba(150,170,210,0.32)",
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid rgba(var(--rgb-cyan),0.26)`,
      }}
    >
      {/* top edge accent */}
      <span
        ...
        style={{
          background:
            "linear-gradient(to right, transparent, rgba(var(--rgb-cyan),0.55), transparent)",
        }}
      />

      {/* ── Titlebar — the single Open action lives here ───────── */}
      <div
        className="flex items-center justify-between gap-3 px-4 md:px-5 py-3 border-b"
      ...
      >
        ...
        <span
          dir="ltr"
          className="ltr-isolate font-mono text-[11px] md:text-xs truncate"
          style={{ color: c.sub }}
        >
          {p.window}
        </span>
      ...
      <div
        className="overflow-y-auto overflow-x-hidden px-3 py-6 md:px-6 md:py-8 max-h-[70vh] md:max-h-[740px]"
        style={{
          backgroundColor: c.canvas,
          scrollbarWidth: "thin",
          scrollbarColor: `${c.thumb} transparent`,
        }}
      >
        ...
        >
          ...
          <header className="text-center space-y-2.5">
            <h3
              className="text-[24px] md:text-[30px] font-bold tracking-tight leading-[1.2]"
              style={{ color: c.ink }}
            >
              {cv.name}
            </h3>
          ...
          <div className="space-y-6">
            ...
            <Section title={s.summary} accent={accent} ink={c.ink} line={c.line}>
              <p
                className="text-[13px] md:text-[14px] leading-[1.85]"
                style={{ color: c.sub }}
              >
            ...
            <Section title={s.education} accent={accent} ink={c.ink} line={c.line}>
              <ul className="space-y-2">
                {cv.education.map((ed) => (
                  <li key={ed.degree}>
                    <p
                      className="text-[13.5px] md:text-[15px] font-semibold leading-snug"
                      style={{ color: c.ink }}
                    >
                      {ed.degree}
                    ...
                    <p
                      className={`mt-1 text-[11px] md:text-[12px] ${meta}`}
                      style={{ color: c.faint }}
                    >
                      {ed.meta}
                    </p>
                ...
                {cv.projects.map((pr) => (
                  <li key={pr.name}>
                    <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5">
                      ...
                      <span
                        className={`text-[11px] md:text-[12px] shrink-0 ${meta}`}
                        style={{ color: c.faint }}
                      >
                        {pr.meta}
                      </span>
                    ...
                    {pr.tech && (
                      <p
                        className="mt-1 text-[11.5px] md:text-[12.5px] italic"
                        style={{ color: c.sub }}
                      >
                        {pr.tech}
                      </p>
            ...
            <Section title={s.skills} accent={accent} ink={c.ink} line={c.line}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                {cv.skills.map((g) => (
                  <div key={g.label}>
                    <p
                      className="text-[11.5px] md:text-[12.5px] font-semibold tracking-wide"
                      style={{ color: c.ink }}
                    >
                      {g.label}
                    </p>
                    <p
                      className="mt-0.5 text-[12px] md:text-[13px] leading-[1.7]"
                      style={{ color: c.sub }}
                    >
          ...
          {/* CV footer */}
          <div
            className="mt-7 pt-3.5 flex items-center justify-between gap-3 border-t"
            style={{ borderColor: c.line }}
          >
            <span
              className={`text-[10px] md:text-[11px] ${meta}`}
              style={{ color: c.faint }}
            >
              {cv.updated}
            </span>
            <span
              dir="ltr"
              className="ltr-isolate font-mono text-[10px] md:text-[11px]"
              style={{ color: c.faint }}
            >
              {PDF_FILENAME}
            </span>
      ...
      <div
        className="flex items-center justify-between gap-3 px-4 md:px-5 py-3 border-t"
        style={{ borderColor: c.bar }}
      >
        <span
          dir="ltr"
          className="ltr-isolate font-mono text-[10px] md:text-[11px] tracking-wide"
          style={{ color: c.faint }}
        >
          {p.documentTag}
        </span>
        <a
          ...
          style={{
            background: warm
              ? "linear-gradient(135deg, #0b6b7d, #0a8ba3)"
              : "linear-gradient(135deg, var(--accent), #00a6cf)",
            color: warm ? "#f4fbfd" : "#04121c",
            boxShadow: warm
              ? "0 6px 16px rgba(11,107,125,0.28)"
              : "0 0 18px rgba(var(--rgb-cyan),0.3)",
          }}
        >
          <svg
            width="13"
  ...
  return (
    <section>
      <div className="flex items-center gap-2.5 mb-3">
        ...
        />
        <h4
          className="text-[12.5px] md:text-[13.5px] font-bold uppercase tracking-[0.08em]"
          style={{ color: ink }}
        >
          {title}
        </h4>
...
function Entry({
  ...
  return (
    <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5">
      ...
      <span
        className={`text-[11px] md:text-[12px] shrink-0 ${metaFont}`}
        style={{ color: faint }}
      >
        {meta}
      </span>
    ...
    <ul className="mt-2 space-y-1.5">
      {items.map((b) => (
        <li key={b} className="flex items-start gap-2.5">
          ...
          />
          <span
            className="text-[12.5px] md:text-[13.5px] leading-[1.75]"
            style={{ color: sub }}
          >
            {b}
          </span>
...
function Contact({
  ...
  const inner = (
    <>
      ...
      <span
        className={href ? "group-hover:underline" : undefined}
