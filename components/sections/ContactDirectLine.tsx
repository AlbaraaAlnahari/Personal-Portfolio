...
import { Mail, Linkedin, Github, Twitter, ArrowUpRight, Copy, Check, Send, Loader2, AlertCircle } from "lucide-react";
...
const CHANNELS = [
  { key: "linkedin", label: "LinkedIn", value: "linkedin.com/in/albaraa-alnahari", href: "https://www.linkedin.com/in/albaraa-alnahari", Icon: Linkedin },
  { key: "github", label: "GitHub", value: "github.com/AlbaraaAlnahari", href: "https://github.com/AlbaraaAlnahari", Icon: Github },
  { key: "x", label: "X / Twitter", value: "x.com/x_ff", href: "https://x.com/x_ff", Icon: Twitter },
] as const;
  ...
  return (
    <div>
      <label htmlFor={id} className="mb-1.5 flex items-center justify-between gap-3">
        ...
        {optional && (
          <span className="font-mono text-[9px] tracking-[0.18em]" style={{ color: MUTED }}>
      ...
      {error && (
        <p id={`${id}-error`} role="alert" className="mt-1.5 text-[12px]" style={{ color: ERR }}>
          {error}
        </p>
      )}
    </div>
...
function ChannelCard({ label, value, href, Icon }: { label: string; value: string; href: string; Icon: typeof Linkedin }) {
  const { t } = useLanguage();
  ...
  return (
    ...
    >
      ...
      <ArrowUpRight aria-hidden="true" size={16} strokeWidth={1.8} className="flex-shrink-0 transition-colors" style={{ color: lit ? CYAN : "var(--text-muted)" }} />
    </a>
  );
}
...
export function ContactDirectLine() {
  ...
  const [form, setForm] = useState({ name: "", email: "", message: "", link: "", company: "" });
  const [topic, setTopic] = useState<Topic>("opportunity");
  const [errors, setErrors] = useState<{ name?: string; email?: string; message?: string; link?: string }>({});
  ...
  function validate() {
    ...
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) e.email = t.contact.form.validation.emailInvalid;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = (await res.json().catch(() => ({}))) as { ok?: boolean; errors?: typeof errors };
      if (res.ok && data.ok) {
        setStatus("success");
      } else {
  ...
  function onTopicKey(e: React.KeyboardEvent, idx: number) {
    let next = idx;
    if (e.key === "ArrowRight" || e.key === "ArrowDown") next = (idx + 1) % TOPIC_KEYS.length;
    else if (e.key === "ArrowLeft" || e.key === "ArrowUp") next = (idx - 1 + TOPIC_KEYS.length) % TOPIC_KEYS.length;
    else return;
    e.preventDefault();
        ...
        <Container style={{ maxWidth: "var(--contact-shell-max)" }}>
          <motion.div initial="hidden" animate="visible" variants={group}>
            ...
            <motion.div variants={fade} className="readability-field">
              ...
              <div className="mt-6 inline-flex items-center gap-2.5 rounded-full px-3.5 py-1.5" style={{ border: `1px solid rgba(var(--rgb-green),0.28)`, backgroundColor: "rgba(var(--rgb-green),0.04)" }}>
                <span aria-hidden="true" className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: "var(--accent-green)", boxShadow: "0 0 8px rgba(var(--rgb-green),0.6)" }} />
                <span className="font-mono text-[10px] tracking-[0.22em]" style={{ color: "rgba(var(--rgb-green),0.9)" }}>
            ...
            <motion.div variants={fade} className="contact-gridwrap relative">
              ...
              <div className="contact-grid grid lg:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)]">
                ...
                <div className="contact-card min-w-0" style={{ border: CARD_BORDER, backgroundColor: SURFACE, boxShadow: CARD_SHADOW }}>
                  ...
                  {status === "success" ? (
                    <div role="status" aria-live="polite" className="flex flex-col items-center px-2 py-10 text-center">
                      <span className="grid h-14 w-14 place-items-center rounded-full" style={{ border: "1px solid rgba(var(--rgb-green),0.4)", backgroundColor: "rgba(var(--rgb-green),0.06)" }}>
                        <Check size={26} strokeWidth={2} aria-hidden="true" style={{ color: "var(--accent-green)" }} />
                      </span>
                      <h3 className="mt-5 text-xl font-bold text-foreground">{t.contact.success.heading}</h3>
                      <p className="mt-2 max-w-sm text-sm leading-relaxed" style={{ color: BODY }}>
                        {t.contact.success.body}
                      </p>
                      {/* Quiet system receipt — fills the panel intentionally without clutter. */}
                      <dl className="mt-5 w-full max-w-xs overflow-hidden rounded-xl border text-left" style={{ borderColor: BORDER_IDLE, backgroundColor: SURFACE_DEEP }}>
                        {[
                          { k: t.contact.success.rows.route.k, v: t.contact.success.rows.route.v, dot: null as string | null },
                          { k: t.contact.success.rows.status.k, v: t.contact.success.rows.status.v, dot: "var(--accent-green)" },
                          { k: t.contact.success.rows.response.k, v: t.contact.success.rows.response.v, dot: null as string | null },
                        ].map((row, i) => (
                          <div
                            key={row.k}
                          ...
                          >
                            ...
                            <dd className="flex items-center gap-1.5 text-[12px] font-medium" style={{ color: "rgba(var(--rgb-body),0.9)" }}>
                              {row.dot && (
                                <span aria-hidden="true" className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: row.dot, boxShadow: `0 0 6px ${row.dot}99` }} />
                              )}
                              {row.v}
                            </dd>
                          </div>
                    ...
                    <form className="space-y-5" noValidate onSubmit={onSubmit}>
                      ...
                      <Field id="cf-message" label={t.contact.form.fields.message.label} as="textarea" value={form.message} onChange={set("message")} placeholder={t.contact.form.fields.message.placeholder} error={errors.message} />
                      <Field id="cf-link" label={t.contact.form.fields.link.label} value={form.link} onChange={set("link")} placeholder={t.contact.form.fields.link.placeholder} optional autoComplete="url" error={errors.link} />

                      {/* Honeypot — hidden from humans; bots that fill it are dropped server-side. */}
                      <div aria-hidden="true" className="absolute h-px w-px overflow-hidden" style={{ left: "-9999px", top: "auto" }}>
                        <label htmlFor="cf-company">{t.contact.form.honeypotLabel}</label>
                      ...
                      <button
                        ...
                        style={{ backgroundColor: "var(--contact-pill-bg)", color: "var(--contact-pill-text)" }}
                        onMouseEnter={(e) => { if (status !== "sending") e.currentTarget.style.backgroundColor = "var(--contact-pill-bg-hover)"; }}
                        onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "var(--contact-pill-bg)"; }}
                      >
                        {status === "sending" ? (
                          <>
                            <Loader2 size={16} strokeWidth={2} aria-hidden="true" className={reduce ? "" : "animate-spin"} />
