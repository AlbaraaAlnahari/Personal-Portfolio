      ...
      <span className="flex items-center gap-2 font-mono text-[10px] tracking-[0.24em]" style={{ color: "rgba(var(--rgb-cyan),0.82)" }}>
        <span
          ...
          style={{
            ...
            boxShadow: lit ? "0 0 8px rgba(var(--rgb-cyan),0.75)" : "0 0 4px rgba(var(--rgb-cyan),0.4)",
          }}
        />
        {section} <span style={{ color: MUTED }}>/ {index}</span>
      </span>
      <span className="font-mono text-[10px] tracking-[0.18em]" style={{ color: MUTED }}>
        {route}
      </span>
    </div>
  );
...
function Cta({ label, lit, className = "" }: { label: string; lit: boolean; className?: string }) {
  return (
    <span className={cn("flex items-center gap-2 font-bold", className)} style={{ color: "var(--accent)" }}>
      {label}
      <span aria-hidden="true" className="transition-transform duration-300" style={{ transform: lit ? "translateX(4px)" : "none" }}>
        →
      </span>
    </span>
  );
...
function GalleryFrameBrackets() {
  ...
  return (
    <>
      <span aria-hidden="true" className={cn(base, "-left-1.5 -top-1.5 border-l border-t")} style={{ borderColor: c }} />
      <span aria-hidden="true" className={cn(base, "-right-1.5 -top-1.5 border-r border-t")} style={{ borderColor: c }} />
      <span aria-hidden="true" className={cn(base, "-bottom-1.5 -left-1.5 border-b border-l")} style={{ borderColor: c }} />
      <span aria-hidden="true" className={cn(base, "-bottom-1.5 -right-1.5 border-b border-r")} style={{ borderColor: c }} />
    </>
  );
}
      ...
      <div className="relative min-h-[180px] md:min-h-[240px] flex-1 overflow-hidden">
        <div className="absolute inset-0 transition-transform duration-500" style={{ transform: lit && !reduce ? "scale(1.03)" : "none" }}>
          ...
          <Image
            ...
            style={{ objectPosition: "center 30%", filter: "brightness(0.88) contrast(1.06) saturate(0.94)" }}
          />
        </div>
        ...
        <div aria-hidden="true" className="pointer-events-none absolute inset-0" style={{ background: "radial-gradient(ellipse at 50% 42%, transparent 46%, rgba(70,38,18,0.08) 70%, rgba(70,38,18,0.2) 100%)" }} />
        <span aria-hidden="true" className="absolute left-3 top-3 h-4 w-4 border-l border-t" style={{ borderColor: "rgba(var(--rgb-cyan),0.5)" }} />
        <span className="absolute right-3 top-3 font-mono text-[9px] tracking-[0.22em]" style={{ color: "rgba(170,185,210,0.6)" }}>
          <span dir="ltr" className="ltr-isolate">{g.profile.idTag}</span>
        </span>
        {/* Bottom navy anchor — slightly lighter so it ties into the card without
            darkening the upper body. */}
        <div aria-hidden="true" className="absolute inset-x-0 bottom-0 h-24" style={{ background: "linear-gradient(to top, rgba(5,9,20,0.6), rgba(5,9,20,0.16), transparent)" }} />
      </div>
      <div className="p-5 md:p-6">
        <MetaRow section={g.profile.section} index="01" route="/about" lit={lit} />
        <h3 className={cn("mt-3 text-xl font-bold text-foreground md:text-2xl", displayFont)}>{g.profile.name}</h3>
        <p className="mt-1.5 text-[13px] font-semibold" style={{ color: "rgba(var(--rgb-cyan),0.85)" }}>
          {g.profile.role}
        </p>
  ...
  return (
    <div {...bind} className={CARD} style={surfaceStyle(lit, reduce)}>
      ...
      <div className="px-5 pt-5 md:px-6 md:pt-6">
        <MetaRow section={w.section} index="02" route="/work" lit={lit} />
        <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-2">
          <h3 className={cn("text-2xl font-bold leading-tight text-foreground md:text-3xl", displayFont)}>{w.name}</h3>
          <span
            className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 font-mono text-[10px] tracking-[0.16em]"
            style={{ color: "rgba(var(--rgb-green),0.95)", border: "1px solid rgba(var(--rgb-green),0.35)", backgroundColor: "rgba(var(--rgb-green),0.06)" }}
          >
            <span aria-hidden="true" className="h-1 w-1 rounded-full" style={{ backgroundColor: "var(--accent-green)" }} />
            {w.award}
          </span>
          <span className="font-mono text-[11px] tracking-[0.06em]" style={{ color: "rgba(var(--rgb-cyan),0.85)" }}>
            {w.platform}
          </span>
        </div>
      </div>
      ...
      <div className="mx-5 mt-4 overflow-hidden rounded-xl border md:mx-6" style={{ borderColor: "var(--border-idle)", backgroundColor: "rgba(8,11,22,0.6)" }}>
        <div className="flex items-center justify-between gap-4 border-b px-4 py-2.5" style={{ borderColor: "var(--border)" }}>
          <span className="font-mono text-[10px] tracking-[0.2em]" style={{ color: MUTED }}>
            {w.deployedInterfaceLabel} · <span dir="ltr" className="ltr-isolate">{w.deployedInterfaceDomain}</span>
          </span>
          <div className="flex items-center gap-4">
            ...
            <span className="inline-flex items-center gap-1.5 font-mono text-[9px] tracking-[0.2em]" style={{ color: "rgba(var(--rgb-green),0.85)" }}>
              <span aria-hidden="true" className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: "var(--accent-green)" }} />
              {w.live}
            </span>
          </div>
        </div>
      ...
      <div className="relative min-h-[180px] md:min-h-[300px] flex-1 overflow-hidden">
        <div className="absolute inset-0 transition-transform duration-500" style={{ transform: lit && !reduce ? "scale(1.03)" : "none" }}>
          <Image
            ...
            className="object-cover"
            style={{ objectPosition: "center 44%", filter: "brightness(1.06) contrast(1.05) saturate(1.1)" }}
          />
        </div>
        ...
        <div aria-hidden="true" className="absolute inset-0" style={{ backgroundColor: "rgba(58,60,140,0.20)", mixBlendMode: "soft-light" }} />
        <div aria-hidden="true" className="absolute inset-0" style={{ background: "radial-gradient(125% 110% at 50% 42%, transparent 58%, rgba(6,9,22,0.24) 100%)" }} />
        <div aria-hidden="true" className="absolute inset-x-0 top-0 h-24" style={{ background: "linear-gradient(to bottom, rgba(6,9,20,0.55) 0%, rgba(6,9,20,0.06) 60%, transparent 100%)" }} />
        <div aria-hidden="true" className="absolute inset-x-0 bottom-0 h-28" style={{ background: "linear-gradient(to top, rgba(8,11,22,0.85) 0%, rgba(8,11,22,0.26) 52%, transparent 100%)" }} />
        <div className="absolute inset-x-0 top-0 p-5 md:p-6">
          <MetaRow section={im.section} index="03" route="/impact" lit={lit} />
        </div>
      </div>
      <div className="p-5 md:p-6">
        <h3 className={cn("text-xl font-bold text-foreground md:text-2xl", displayFont)}>{im.name}</h3>
        <div className="mt-3 flex items-baseline gap-3">
          <span className="text-4xl font-bold leading-none tabular-nums md:text-5xl" style={{ color: "var(--accent)", textShadow: "0 0 22px rgba(var(--rgb-cyan),0.28)" }}>
            {im.metric}
          </span>
          <span className="font-mono text-[11px] tracking-[0.2em]" style={{ color: "rgba(var(--rgb-muted),0.75)" }}>
            {im.metricLabel}
          </span>
        </div>
        <p className="mt-2 text-sm leading-relaxed" style={{ color: BODY }}>
          <span className="font-semibold" style={{ color: "var(--text-strong)" }}>{im.roleStrong}</span>{im.roleRest}
        </p>
...
function ResumeCard({ reduce }: { reduce: boolean | null }) {
  ...
  return (
    <div {...bind} className={cn(CARD, "p-5 md:p-6")} style={surfaceStyle(lit, reduce)}>
      <MetaRow section={r.section} index="04" route="/resume" lit={lit} />
      <h3 className={cn("mt-3 text-xl font-bold text-foreground", displayFont)}>{r.name}</h3>
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
        <Link href="/resume" aria-label={r.ctaAria} className="ie-focus rounded outline-none after:absolute after:inset-0 after:z-[1] after:content-['']">
...
function ContactCard({ reduce }: { reduce: boolean | null }) {
  ...
  return (
    <Link href="/contact" aria-label={c.cardAria} {...bind} className={cn(CARD, "p-5 md:p-6")} style={surfaceStyle(lit, reduce)}>
      <MetaRow section={c.section} index="05" route="/contact" lit={lit} />
      <h3 className={cn("mt-3 text-xl font-bold text-foreground", displayFont)}>{c.name}</h3>
      <p className="mt-1.5 text-sm leading-relaxed" style={{ color: BODY }}>
        {c.description}
      </p>
...
export function DestinationGallery() {
  ...
  const item: Variants = {
    ...
    visible: { opacity: 1, y: 0, transition: { duration: reduce ? 0 : 0.55, ease: [0.4, 0, 0.2, 1] } },
  };
    ...
    <section className="relative overflow-hidden pb-10 pt-16 md:pb-14 md:pt-28">
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
        <Image
          ...
          style={{ left: "-115px", top: "52%", opacity: 0.10, filter: "brightness(1.3)" }}
        />
        <Image
          src="/images/232.svg"
          alt=""
          ...
          style={{ right: "-48px", bottom: "-30px", opacity: 0.17, filter: "brightness(1.5)" }}
        />
      </div>
      ...
      <Container>
        ...
        <motion.div className="readability-field" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.4 }} variants={item}>
          <div className="mb-6 flex items-center gap-3">
            <span aria-hidden="true" className="h-px w-8" style={{ background: "linear-gradient(to right, transparent, rgba(var(--rgb-cyan),0.6))" }} />
            <span aria-hidden="true" className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: "var(--accent)", boxShadow: "0 0 6px rgba(var(--rgb-cyan),0.6)" }} />
            <span className="font-mono text-[10px] tracking-[0.28em]" style={{ color: "rgba(var(--rgb-cyan),0.75)" }}>
              {g.enterArchive}
            </span>
          </div>
          <p className="font-mono text-[11px] tracking-[0.24em]" style={{ color: "rgba(var(--rgb-body),0.62)" }}>
            {g.personalArchive} <span style={{ color: MUTED }}>/ {g.destinationGallery}</span>
          </p>
          <h2 className={cn("mt-4 max-w-2xl text-3xl font-bold leading-[1.1] text-foreground md:text-4xl lg:text-[2.75rem]", displayFont)}>
            {g.title}
          </h2>
          <p className="mt-4 max-w-xl text-base leading-relaxed" style={{ color: "var(--text-body)" }}>
            {g.subtitle}
          </p>
        </motion.div>
        ...
        <div className="relative mt-5 md:mt-8">
          ...
          <div className="mb-5 flex items-center gap-3 md:mb-6">
            <span className="flex items-center gap-2 font-mono text-[10px] tracking-[0.24em]" style={{ color: "rgba(200,210,228,0.6)" }}>
              <span aria-hidden="true" className="h-1 w-1 rounded-full" style={{ backgroundColor: "rgba(200,210,228,0.7)" }} />
              {g.archiveField}
            </span>
            <span aria-hidden="true" className="h-px flex-1" style={{ background: "linear-gradient(to right, rgba(226,232,242,0.2), rgba(226,232,242,0.05))" }} />
            <span className="font-mono text-[10px] tracking-[0.2em]" style={{ color: MUTED }}>{g.destinationsCount}</span>
          </div>
